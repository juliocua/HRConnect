...
const TYPE_META: Record<ImportType, { label: string; icon: string; desc: string; fields: { col: string; desc: string; required?: boolean; values?: string }[] }> = {
  ...
  employee: {
    ...
    fields: [
      ...
      { col: 'clientName', desc: 'Client company where employee is deployed — must exactly match an existing client name (optional)' },
      ...
      { col: 'sssNo', desc: 'SSS number' },
      { col: 'philhealthNo', desc: 'PhilHealth number' },
      { col: 'pagibigNo', desc: 'Pag-IBIG / HDMF number' },
      { col: 'tinNo', desc: 'Tax Identification Number (TIN)' },
    ],
  },
  time: {
    ...
    fields: [
      ...
      { col: 'timeIn', desc: 'Time in (for PRESENT, LATE, HALF_DAY)', values: 'HH:MM (24-hour, e.g. 08:30)' },
      { col: 'timeOut', desc: 'Time out (for PRESENT, LATE, HALF_DAY)', values: 'HH:MM (24-hour, e.g. 17:00)' },
