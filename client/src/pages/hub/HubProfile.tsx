...
import type { Employee, ProfileChangeRequest, GovIdChangeRequest } from '@/types';
...
// ── Government IDs form (change-request flow) ─────────────────────────────────
function GovtIdsForm({ employee }: { employee: Employee }) {
  const qc = useQueryClient();
  const toast = useToast();

  const { data: requests } = useQuery<GovIdChangeRequest[]>({
    queryKey: ['my-gov-id-requests'],
    queryFn: () => api.get('/employees/me/gov-id-requests').then(r => r.data),
  });
  ...
  const submitMutation = useMutation({
    mutationFn: (data: typeof form) => api.post('/employees/me/gov-id-request', data),
    onSuccess: () => {
      setSuccess(true);
      toast('success', 'Gov ID change request submitted');
      qc.invalidateQueries({ queryKey: ['my-gov-id-requests'] });
    },
    onError: (err: any) => {
  ...
  return (
    <div>
      ...
      <div className="form-grid form-grid-2" style={{ gap: 12, marginBottom: 16 }}>
        {(['sssNo', 'philhealthNo', 'pagibigNo', 'tinNo'] as const).map(key => (
          <div key={key} style={{ fontSize: 13 }}>
            <div style={{ fontWeight: 600, color: 'var(--color-text-muted)', marginBottom: 2, fontSize: 12 }}>
              {GOV_ID_LABELS[key]}
            </div>
            <div style={{ fontFamily: 'monospace', color: employee[key] ? 'var(--color-text-primary)' : 'var(--color-text-muted)' }}>
              {employee[key] ?? <span style={{ fontStyle: 'italic', fontFamily: 'inherit' }}>Not on file</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      {hasPending && latest && (
        <div style={{
          ...
          {(['sssNo', 'philhealthNo', 'pagibigNo', 'tinNo'] as const).filter(k => latest[k]).map(k => (
            <div key={k} style={{ fontSize: 12, color: '#92400E', marginTop: 2 }}>
              <strong>{GOV_ID_LABELS[k]}:</strong> {latest[k]}
            </div>
          ))}
          <div style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 6 }}>
            Your current values are shown above. Changes will apply once approved by HR.
          </div>
        </div>
      )}
      ...
      <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 14 }}>
        Changes to government IDs require HR verification. Only fill in the IDs you want to update.
        Submitting a new request will supersede any pending one.
      </p>
